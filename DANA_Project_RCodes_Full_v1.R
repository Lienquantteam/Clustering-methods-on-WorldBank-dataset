...
library(tidyverse)
library(cluster)
setwd('C:/Users/LIEN PHAM/OneDrive - Langara College/Desktop/Langara/DANA 4840/Project')

df_final= read.csv('df_noout (2).csv')
View(df_final)
...
max(sw_temp) # 0.3555782(Shihoue score of Kmedoid method), has 2 meanings: k =2, method = kmedoid
...
sw_single<-numeric() #single
sw_complete<-numeric() #complete
sw_average<-numeric() #hc_average
sw_wardD2<-numeric() # wardD2

ch_single<-numeric() #single
ch_complete<-numeric() #complete
ch_average<-numeric() #hc_average
ch_wardD2<-numeric() # wardD2

for (i in 2:10)
{
  sw<-silhouette(cutree(hc_single, k = i), eu)
  sw_single<-c(sw_single, mean(sw[,3]))
  ch_single<-c(ch_single, fpc::calinhara(eu, cutree(hc_single, i)))
}

for (i in 2:10)
{
  sw<-silhouette(cutree(hc_complete, k = i), eu)
  sw_complete<-c(sw_complete, mean(sw[,3]))
  ch_complete<-c(ch_complete, fpc::calinhara(eu, cutree(hc_complete, i)))
}

for (i in 2:10)
{
  sw<-silhouette(cutree(hc_average, k = i), eu)
  sw_average<-c(sw_average, mean(sw[,3]))
  ch_average<-c(ch_average, fpc::calinhara(eu, cutree(hc_average, i)))
}

for (i in 2:10)
{
  sw<-silhouette(cutree(hc_wardD2, k = i), eu)
  sw_wardD2<-c(sw_wardD2, mean(sw[,3]))
  ch_wardD2<-c(ch_wardD2, fpc::calinhara(eu, cutree(hc_wardD2, i)))
}

# cbind all sw_list
sw_list<-cbind(sw_single, sw_complete, sw_average, sw_wardD2)
sw_list
#       sw_single sw_complete sw_average sw_wardD2
#[1,]  0.24431939   0.3696676  0.3635434 0.3694468 -> wardD2 has highest scores with k =2
...
#sw_list1 #  0.3694468 0.2421877 0.2148586 0.1850567 0.1846749 0.1763091 0.1831650 0.1528211 0.1499510
ch_list = cbind(ch_single,ch_complete,ch_average,ch_wardD2)
ch_list

#      ch_single ch_complete ch_average ch_wardD2
#[1,]  2.347684   161.43729  158.90334 164.36272 -> ward D2 has highest CH score, with k =2
#[2,]  5.208947   141.65347   85.03813 158.67000
#[3,]  4.881848   104.08367  101.29612 131.65109
#[4,]  3.729960    99.34525   98.97265 132.15571
#[5,]  4.102568   102.85451   88.74460 118.24940
#[6,]  3.454631    89.81414   74.51522 102.26244
#[7,]  3.088748    87.04003   64.16104  91.73435
#[8,]  2.705327    93.48398   58.24927  92.97035
#[9,]  2.426490    88.73290   52.33390  90.97680
...
fviz_dend(hc_wardD2, k = 2, # Cut in 2 groups
          cex = 0.5, # label size
...
fviz_dend(hc_wardD2, cex = 0.5, k = 2,
          k_colors = "jco", type = "circular")

require("igraph")
fviz_dend(hc_wardD2, k = 2, k_colors = "jco",
